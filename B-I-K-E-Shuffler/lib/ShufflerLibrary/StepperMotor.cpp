 ...
 */
void StepperMotor::moveToTarget(long targetStep)
{
    writeInfo("Moving to target: " + String(targetStep));
    setDirection(targetStep > this->current); // True -> CW, False -> CCW
    long stepsToMove = abs(targetStep - this->current);
...
void StepperMotor::moveToTargetAccel(long targetStep)
{
    writeInfo("Moving to target: " + String(targetStep));
    ...
    long minSpeed = maxSpeed / 3;
    long diff = maxSpeed - minSpeed;
    setDirection(targetStep > this->current); // True -> CW, False -> CCW
    long stepsToMove = abs(targetStep - this->current);
    long quarterSteps = stepsToMove / 4;
    for (long i = 0; i < stepsToMove / 4; i++) {
        setSpeed(diff * (float(i) / quarterSteps) + minSpeed);
        stepMotor();
    }

    for (long i = 0; i < stepsToMove / 2 + stepsToMove % 4; i++) {
        setSpeed(maxSpeed);
        stepMotor();
    }

    for (long i = 0; i < stepsToMove / 4; i++) {
        setSpeed(diff * (1.0 - (float(i) / quarterSteps)) + minSpeed);
        stepMotor();
    }
